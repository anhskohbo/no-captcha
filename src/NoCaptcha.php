...
class NoCaptcha
{
    ...
    protected function sendRequestVerify(array $query = [])
    {
        ...
        if ($proxy_url) {
            $parts = explode('@', substr($proxy_url, 7), 2);
            ...
            if (count($parts) == 2) {
                $options['http']['header'] = base64_decode($parts[0]);
            }

            $proxyContext = stream_context_create($options);

            $response = file_get_contents($link, true, $proxyContext);
        } else {
